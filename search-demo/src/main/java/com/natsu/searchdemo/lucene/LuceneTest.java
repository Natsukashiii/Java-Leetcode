package com.natsu.searchdemo.lucene;
...
public class LuceneTest {
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.printf("当前共有%d条数据%n", productNames.size());
    System.out.printf("查询关键字为::\"%s\"%n", keyword);
    ScoreDoc[] hits = searcher.search(query, numberPerPage).scoreDocs;

    //5. 显示查询结果
    showResearchResult(searcher, hits, query, analyzer);

    //6. 关闭查询
    reader.close();
  }

  public static void showResearchResult(IndexSearcher searcher, ScoreDoc[] hits, Query query,
    ...
    for (int i = 0; i < hits.length; ++i) {
      ...
      for (IndexableField field : fields) {
        System.out.println("\t" + document.get(field.name()));
      }
      System.out.println();
    }
  }
  ...
  public static void addDoc(IndexWriter writer,String name )throws IOException {
